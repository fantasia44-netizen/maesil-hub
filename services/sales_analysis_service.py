...
def _fetch_month_actual_production(db, first, last):
    """stock_ledger에서 월별 실제 생산/소분 수량 조회.

    Returns:
        dict: {'PRODUCTION': total_qty, 'REPACK_IN': total_qty}
    """
    result = {'PRODUCTION': 0, 'REPACK_IN': 0}
    try:
        for t in ('PRODUCTION', 'REPACK_IN'):
            # 페이지네이션 (대량 데이터 대응)
            offset, total = 0, 0
            while True:
                res = db.client.table('stock_ledger').select('qty') \
                    .eq('type', t) \
        ...
        logging.getLogger(__name__).warning(
            f'[SALES-ANALYSIS] stock_ledger 실적 조회 실패: {e}'
        )
    return result
...
def get_monthly_sales_analysis(db, year=None, month=None):
    ...
    # ── cost_type '생산'/'소분' 카드는 판매량이 아닌 실제 실적으로 교체 ──
    # 판매량 기반은 오해 소지 → stock_ledger(PRODUCTION/REPACK_IN)에서 월합계 조회
    cur_first, cur_last, _ = _month_range(year, month)
    prev_first, prev_last, _ = _month_range(py, pm)
    cur_act = _fetch_month_actual_production(db, cur_first, cur_last)
    prev_act = _fetch_month_actual_production(db, prev_first, prev_last)

    _ACTUAL_MAP = {'생산': 'PRODUCTION', '소분': 'REPACK_IN'}
    for ct in cost_type_totals:
        cat = ct.get('category', '')
        if cat not in _ACTUAL_MAP:
            continue
        key = _ACTUAL_MAP[cat]
        ct['cur_qty'] = cur_act.get(key, 0)
        ct['prev_qty'] = prev_act.get(key, 0)
        # 일평균/달성율 재계산
        daily_c = ct['cur_qty'] / elapsed if elapsed > 0 else 0
        daily_p = ct['prev_qty'] / prev_days if prev_days > 0 else 0
        ct['achievement_rate'] = round(daily_c / daily_p * 100, 1) if daily_p > 0 else None
        # 금액은 판매량과 연동되므로 그대로 유지 (생산/소분 실적 금액 없음)

    # category_totals 하위 호환은 cost_type_totals 동일 객체 공유
