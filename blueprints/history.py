...
# type → 원본 페이지 매핑
_TYPE_TO_SOURCE = {
    'PRODUCTION': ('생산관리', 'production.index'),
    'PROD_OUT':   ('생산관리', 'production.index'),
    'ADJUSTMENT': ('재고조정', 'adjustment.index'),
    'REPACK_IN':  ('소분', 'repack.index'),
    'REPACK_OUT': ('소분', 'repack.index'),
    'INBOUND':    ('입고', 'inbound.index'),
}
...
@role_required('admin', 'manager', 'logistics', 'production', 'general')
def edit(row_id):
    """이력 페이지에서는 수정 불가 — type에 따라 원본 페이지로 안내"""
    try:
        rec = get_db().query_stock_ledger_by_id(row_id)
        rec_type = (rec or {}).get('type', '') if rec else ''
        src = _TYPE_TO_SOURCE.get(rec_type)
        if src:
            source_name, endpoint = src
            flash(
                f'이력 페이지에서는 수정할 수 없습니다. '
                f'이 레코드는 [{source_name}]에서 입력된 것이므로 '
                f'[{source_name}] 페이지에서 수정하세요.',
                'warning'
            )
            try:
            ...
            flash(
                '이력 페이지에서는 수정할 수 없습니다. '
                '원본 페이지(생산관리/재고조정/입고/소분)에서 수정하세요.',
                'warning'
            )
    except Exception:
        flash('이력 페이지에서는 수정할 수 없습니다.', 'warning')
