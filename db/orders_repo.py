...
class OrdersRepo(BaseRepo):
    ...
    def _upsert_order_batch_fallback(self, import_run_id, orders):
        ...
        for batch_start in range(0, len(orders), BATCH):
            ...
            for ch, order_nos in by_channel.items():
                # ★ RPC 우선: rpc_check_order_no_exists (1000행 limit 회피)
                try:
                    ...
                    # Fallback: chunk IN 조회 (1000행 limit 회피)
                    onos = list(order_nos)
                    CHUNK = 500
                    try:
                        for ci in range(0, len(onos), CHUNK):
                            ch_onos = onos[ci:ci + CHUNK]
                            res = self.client.table("order_transactions") \
                                .select("id,channel,order_no,line_no,raw_hash,status") \
                                .eq("channel", ch) \
                                .in_("order_no", ch_onos) \
            ...
            if batch_hashes:
                ...
                try:
                    ...
                    # Fallback: chunk IN 조회 (range 명시로 limit 회피)
            ...
            if batch_order_nos:
                onos_list = list(batch_order_nos)
                # ★ chunk + range로 1000행 limit 회피
                for oi in range(0, len(onos_list), 200):
                    o_chunk = onos_list[oi:oi + 200]
                    try:
    ...
    def query_orders_by_import_run(self, import_run_id, outbound_done=None):
        ...
        ★ 1000행 limit 회피: range 페이지네이션 적용
        """
        try:
            all_rows = []
            offset = 0
    ...
    def get_import_run_impact(self, run_id):
        ...
        ★ RPC rpc_get_import_run_summary 우선 사용 (1000행 limit 회피).
        ...
        try:
            ...
            # RPC 우선: 카운트가 정확함 (페이지네이션 무관)
            try:
                rpc_res = self.client.rpc('rpc_get_import_run_summary',
                                          {'p_run_id': run_id}).execute()
                summary = rpc_res.data or {}
                if summary:
                    total = summary.get('total', 0)
                    active = summary.get('active_count', 0)
                    outbound = summary.get('outbound_done_count', 0)
                    return {
                        "run": run,
                        "total_count": total,
                        "active_count": active,
                        "outbound_count": outbound,
                        "already_cancelled": total - active,
                    }
            except Exception as e:
                print(f"[DB] rpc_get_import_run_summary fallback: {e}")

            # Fallback: 페이지네이션 직접 조회
            all_list = []
            offset = 0
            PAGE = 1000
            while True:
    ...
    def cancel_import_run(self, run_id, cancelled_by):
        ...
        try:
            ...
            # 2) 해당 run의 정상 주문 조회 (★ 페이지네이션으로 1000행 limit 회피)
            active_orders = []
            offset = 0
            PAGE = 1000
            while True:
                ...
                try:
                    ...
                    while True:
                        sl_res = self.client.table("stock_ledger") \
                            ...
                            .range(sl_offset, sl_offset + 999).execute()
                        sl_rows = sl_res.data or []
                        if not sl_rows: break
                        sl_ids.extend(r["id"] for r in sl_rows)
                        if len(sl_rows) < 1000: break
                        sl_offset += 1000
                    for sl_id in sl_ids:
