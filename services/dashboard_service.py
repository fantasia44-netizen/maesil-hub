...
def get_dashboard_data(db, date=None, force_refresh=False):
    ...
    # ── 1차: 통합 RPC 1번 호출 (매실 인사이트 패턴) ──
    # 실패 시 아래 ThreadPool 폴백으로 자동 전환
    try:
        rpc_res = db.client.rpc('get_dashboard_full', {
            'p_date': date,
        ...
        if rpc_data and rpc_data.get('kpi'):
            warehouse_stock = rpc_data.get('warehouse_stock') or []
            total_products = sum(s.get('product_count', 0) for s in warehouse_stock)
            kpi = rpc_data.get('kpi') or {}
            kpi['stock_products'] = total_products

            # outbound_summary 의 pending/done 도 KPI 에 머지
            ob = rpc_data.get('outbound_summary') or {}
            kpi['outbound_pending'] = int(ob.get('pending', 0) or 0)
            kpi['outbound_done'] = int(ob.get('done', 0) or 0)
            ...
            result = {
                ...
                'kpi': kpi,
                'revenue_trend': rpc_data.get('revenue_trend') or [],
                'channel_breakdown': rpc_data.get('channel_breakdown') or [],
                'warehouse_stock': warehouse_stock,
                'top_products': rpc_data.get('top_products') or [],
                'recent_activity': [],  # 별도 (RPC 미포함, 가벼움)
            }
            # recent_activity 는 가볍게 추가 1회
            try:
                result['recent_activity'] = db.query_recent_activity(limit=15) or []
            except Exception:
                pass
        ...
        print(f'[Dashboard] get_dashboard_full RPC 실패 → ThreadPool 폴백: {e}')

    # ── 2차 (폴백): 독립 쿼리 6개를 병렬 실행 ──
    results = {}
