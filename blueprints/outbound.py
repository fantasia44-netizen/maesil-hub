...
def index():
    ...
    # 거래 이력 조회 (manual_trades + stock_ledger SALES_OUT 통합)
    if date_from or date_to or (partner_filter and partner_filter != '전체') or keyword:
        try:
            # 1) 거래처 직접 출고 (manual_trades)
            manual = db.query_manual_trades(
            ...
            if keyword or date_from or date_to:
                try:
                    ...
                    for r in raw:
                        ...
                        if kw_norm and kw_norm not in pname.replace(' ', '').lower():
                            continue
                        sales_out.append({
                            'id': r.get('id'),
                            'trade_date': r.get('transaction_date', ''),
                            'partner_name': r.get('memo', '') or '-',
                            'product_name': pname,
                            'qty': abs(r.get('qty', 0)),
                            'unit_price': 0,
                            'amount': 0,
                            'memo': f"주문출고 ({r.get('location', '')})",
                            '_source': 'sales_out',
                        })
                except Exception:
                    pass
            ...
            trade_list = sorted(
                manual + sales_out,
                key=lambda x: str(x.get('trade_date') or ''),
                reverse=True,
            )
