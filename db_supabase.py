...
# ──────────────────────────────────────────────────────────
if not getattr(httpx, '_maesil_http1_forced', False):
    ...
    httpx._maesil_http1_forced = True
...
class SupabaseDB(DBBase):
    ...
    def query_stock_by_location(self, location, select_fields=None, product_names=None, biz_id=None):
    ...
    def upsert_revenue(self, payload_list, skip_auto_stock_categories=None, *, biz_id=None):
        ...
        # ★ batch 내 conflict key 중복 사전 dedup (canonical 강제로 발생 가능)
        #   on_conflict='revenue_date,product_name,category,channel,invoice_no' 인데
        #   같은 batch에 같은 키가 2개면 ON CONFLICT 실패 (PG 21000)
        #   → qty/revenue 합산하여 1개 row로 머지
        from collections import OrderedDict
        merged = OrderedDict()
        for p in payload_list:
            key = (
                p.get('revenue_date') or '',
            ...
            if key in merged:
                existing = merged[key]
                # 합산 가능한 수치 필드만 누적, 나머지는 기존 보존
                for fld in ('qty', 'revenue', 'unit_price'):
                    if fld == 'unit_price':
                        # unit_price 는 평균/마지막 — 마지막 값 유지 (보통 동일)
                        existing[fld] = p.get(fld, existing.get(fld, 0))
                    else:
                        try:
                            existing[fld] = (existing.get(fld, 0) or 0) + (p.get(fld, 0) or 0)
                        except (TypeError, ValueError):
                            pass
            else:
                merged[key] = dict(p)
        deduped = list(merged.values())
        if len(deduped) < len(payload_list):
            print(f'[upsert_revenue] batch dedup: {len(payload_list)} → {len(deduped)} (conflict key 중복 머지)')

        self.client.table("daily_revenue").upsert(
            deduped, on_conflict="revenue_date,product_name,category,channel,invoice_no"
        ).execute()
    ...
    def _auto_stock_from_revenue(self, payload_list, skip_categories=None, *, biz_id=None):
        ...
        안전성:
          ...
          - skip_categories: 호출측에서 직접 stock_ledger 처리하는 카테고리 제외 (이중 차감 방지)
        """
        from services.product_name import canonical
        skip_set = set(skip_categories or [])
        targets = []
        for p in payload_list:
            ...
            if cat not in ('로켓', '거래처매출'):
                continue
            if cat in skip_set:
                continue  # 호출측에서 process_single_outbound 등으로 직접 처리
        ...
        for (date, pn, wh), total_qty in grouped.items():
            cat = cats[(date, pn, wh)]
            event_uid = f'DR_AUTO:{date}:{wh}:{pn}:{cat}'
            try:
                # ── 이중차감 방지: event_uid 기반 idempotent 체크 ──
                # 2026-04-16 수정: 당시 `date+product_name`만으로 체크했더니
    ...
    def _invalidate_price_cache(self):
        """master_prices 변경 후 price_map 캐시 무효화 (가격등록 직후 0원 저장 사고 방지)."""
        self._price_cache['data'] = None
        self._price_cache['ts'] = 0

    def query_price_table(self, use_cache=True):
        """가격표(master_prices) 전체 조회 → aggregator.price_map 호환 dict 반환.

        키는 (1) 원본명 (2) canonical(공백제거) 양쪽 모두 등록 →
        호출측이 어떤 형태로 lookup해도 매칭됨 (띄어쓰기 사고 방지).
        """
        import unicodedata
        from services.product_name import canonical
        now = time.time()
        ...
        for r in rows:
            ...
            price_map[nm] = entry
            # canonical(공백제거) 키도 함께 등록 — 양방향 매칭 보장
            cnm = canonical(nm)
            if cnm and cnm != nm and cnm not in price_map:
                price_map[cnm] = entry
    ...
    def rename_product(self, old_name, new_name, *, biz_id=None):
        ...
        전파 대상:
          - product_costs       (마스터: INSERT 신규 + DELETE 기존)
          - stock_ledger        (재고 이력: UPDATE)
          - daily_revenue       (매출: UPDATE)
          - order_transactions  (주문: UPDATE)
          - option_master       (옵션 매핑: UPDATE)
          - products            (신규 마스터, 있으면: UPDATE)
          - price_master        (가격 마스터, 있으면: UPDATE)
          - bom                 (부모/자식 품목명, 있으면: UPDATE)

        제외:
          - api_orders.raw_data (JSONB 내부라 자동 변환 불가, 필요 시 수동)
        ...
        if not old or not new:
            return {'success': False, 'error': '품목명이 비어있음'}
        if old == new:
        ...
        try:
            ...
            if exists.data:
                return {
                    'success': False,
                    'error': f'"{new}" 품목이 이미 존재합니다. 다른 이름 사용 또는 기존 품목 정리 후 재시도.'
                }
        except Exception as e:
            return {'success': False, 'error': f'중복 체크 실패: {e}'}
        ...
        try:
            ...
            errors.append(f'product_costs: {str(e)[:200]}')
            # 실패 시 즉시 중단 — 나머지 UPDATE 진행하면 마스터와 이력 불일치 악화
            return {'success': False, 'error': errors[0], 'changed': changed,
                    'errors': errors}

        # 3) 이력/참조 테이블 단순 UPDATE (존재하지 않는 테이블은 조용히 스킵)
        simple_tables = [
            'stock_ledger',
            'daily_revenue',
            'order_transactions',
            ...
            'products',
            'price_master',
            'bom',  # parent_product_name 또는 child_product_name — 아래에서 별도 처리
        ]
        for tbl in simple_tables:
            ...
            try:
                ...
                if '42P01' in msg or 'does not exist' in msg.lower() \
                   or 'relation' in msg.lower() and 'not' in msg.lower():
                    continue  # 테이블 없음은 스킵
                errors.append(f'{tbl}: {msg[:200]}')

        # 4) bom: parent/child 양쪽 업데이트 (테이블 있을 때)
        try:
    ...
    def get_cj_sender(self) -> dict:
        """CJ 발송인 정보 — my_business 기본 사업장 기준.
        cj_sender_* 컬럼 우선, 없으면 business_name/phone/address 폴백.
        컬럼이 아직 없는 환경(마이그레이션 미적용)은 빈 dict 반환.
        """
        try:
            biz = self.query_default_business()
            if not biz:
                return {}
            return {
                'name':           biz.get('cj_sender_name')    or biz.get('business_name', ''),
                'phone':          biz.get('cj_sender_phone')   or biz.get('phone', ''),
                'zipcode':        biz.get('cj_sender_zipcode', ''),
                'address':        biz.get('cj_sender_address') or biz.get('address', ''),
                'detail_address': biz.get('cj_sender_detail',  ''),
            }
        except Exception:
            return {}
    ...
    def insert_option_master_batch(self, payload_list, batch_size=500, *, biz_id=None):
        ...
        for row in payload_list:
            ...
            if row.get('product_name'):
                row['product_name'] = canonical(row['product_name'])

        # ★ batch 내 match_key 중복 dedup (canonical/normalize 후 중복 가능)
        from collections import OrderedDict
        merged = OrderedDict()
        for row in payload_list:
            mk = row.get('match_key', '')
            if mk:
                merged[mk] = row  # 마지막 값 유지
            else:
                merged[id(row)] = row  # match_key 없으면 별개 취급
        deduped = list(merged.values())
        if len(deduped) < len(payload_list):
            print(f'[insert_option_master_batch] dedup: {len(payload_list)} → {len(deduped)}')
        payload_list = deduped
    ...
    def update_option_master(self, option_id, update_data, biz_id=None):
        """옵션마스터 1건 수정. product_name은 canonical 강제."""
        from services.product_name import canonical
        if 'original_name' in update_data:
            update_data['match_key'] = normalize_match_key(update_data['original_name'])
        if update_data.get('product_name'):
            update_data['product_name'] = canonical(update_data['product_name'])
    ...
    def replace_stock_ledger(self, old_id, new_payload, replaced_by_user=None, biz_id=None):
        ...
        원자성 보강 (2026-04-23): 새 레코드 INSERT 실패 시 원본 status 를 원복한다.
        이전에는 status='replaced' 만 먼저 업데이트된 뒤 INSERT가 23505 UNIQUE
        충돌로 실패하면 원본이 죽은 상태로 남아 재고가 사라지는 사고 발생.
        ...
        # 원본의 원래 status 보존 (롤백용)
        try:
            orig_res = self.client.table("stock_ledger").select("status") \
                .eq("id", old_id).limit(1).execute()
            orig_status = (orig_res.data[0].get('status') if orig_res.data
                           else 'active') or 'active'
        except Exception:
            orig_status = 'active'
        ...
        # 2) 새 레코드 INSERT (event_uid 제거 권장 — UNIQUE 충돌 차단)
        new_payload['replaces'] = old_id
        new_payload['created_by'] = replaced_by_user
        new_payload['status'] = 'active'
        # INSERT 실패해도 원본 event_uid 충돌은 없도록 new_payload 에서 제거
        new_payload.pop('event_uid', None)

        try:
            filtered = self._filter_payload([new_payload])
            self._normalize_product_names(filtered)  # 공백제거+products 마스터 동기화
            ...
            try:
                ...
                print(f'[replace_stock_ledger] INSERT 실패 → 원본({old_id}) '
                      f'status 복구 ({orig_status}): {ins_err}')
            except Exception as rb_err:
                print(f'[replace_stock_ledger] ⚠️ 롤백도 실패 (old_id={old_id}): '
                      f'ins={ins_err} rb={rb_err}')
            raise
    ...
    def query_orders_by_channel(self, date_from=None, date_to=None, biz_id=None):
    ...
    def query_stock_summary_by_location(self, exclude_products=None, biz_id=None):
        ...
        biz_id가 있으면 폴백 경로에서 필터 적용 (RPC는 전체 조회).
        """
        # RPC 경로 (exclude_products가 비어있고 biz_id 없을 때만)
        if not exclude_products and not biz_id:
            try:
        ...
        try:
            ...
            while True:
                try:
                    q = self.client.table("stock_ledger") \
                        .select("product_name,location,qty") \
                        .gte("transaction_date", date_from) \
                        .lte("transaction_date", today)
                    q = self._with_biz(q, biz_id)
                    res = q.order("id").range(offset, offset + page_size - 1).execute()
                    rows = res.data or []
                except Exception as e:
                    if self._is_connection_error(e):
                        self._reconnect()
