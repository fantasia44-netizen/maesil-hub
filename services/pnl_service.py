...
def _fetch_month_data(db, date_from, date_to, year_month, maesil_sb=None, maesil_op_id=None):
    ...
    RPC 경로에서는 _rpc 데이터 대신 폴백 경로를 사용해 Maesil 정산 반영.

    구 버전: 4개 테이블 Python 풀스캔 + defaultdict 4중 집계 (피크 ~6MB/월).
    신 버전: get_pnl_monthly_agg RPC 1회 호출, JSONB 파싱만 수행.
    """
    # Maesil 정산 데이터 미리 조회 (브릿지 활성 시)
    # RPC 사용: RESERVE/revenue_ 자동 처리, Maesil 대시보드와 동일한 집계
    maesil_settlements = []
    if maesil_sb and maesil_op_id:
        from services.maesil_bridge import get_maesil_settlements_by_month
        maesil_settlements = get_maesil_settlements_by_month(maesil_sb, maesil_op_id, year_month)
    ...
    # Maesil 데이터가 있으면 병합 경로 사용 (RPC는 maesil-total 자체 테이블만 읽으므로 스킵)
    # Maesil 없을 때도 settlements는 항상 로드 — promo_deduction(쿠폰/포인트) 계산에 필요
    own_settlements = db.query_api_settlements(date_from=date_from, date_to=date_to) or []

    if agg and not maesil_settlements:
        # RPC 성공 + Maesil 없음: 매출/원가는 RPC 결과 사용, settlements는 promo_deduction용
        return {'_rpc': agg, 'settlements': own_settlements}

    if agg and maesil_settlements:
        logger.info('[P&L] Maesil 정산 데이터 있음 — RPC 결과 대신 병합 경로 사용')

    # 폴백: 기존 Python 경로 + Maesil 병합
    if maesil_settlements:
        from services.maesil_bridge import merge_settlements
        own_settlements = merge_settlements(own_settlements, maesil_settlements)

    return {
        '_rpc': None,
        'settlements': own_settlements,
...
# revenue_ 는 Maesil 쿠팡 Wing 일별매출 기록 — wsettle_(주별정산)과 중복이므로 P&L에서 제외
_SETTLE_PREFIXES = (
...
def _calc_revenue_v2(db, date_from, date_to, data):
    ...
    online_deductions = 0   # 쿠폰+포인트+기타차감 합계 (계산서 없는 차감)
    online_by_channel_net = defaultdict(int)  # net_settlement 채널별 (참고용)
    ...
    for s in settlements:
        ...
        net = int(s.get('net_settlement') or 0)
        comm = int(s.get('total_commission') or 0)
        coupon = int(s.get('coupon_discount') or 0)
        point = int(s.get('point_discount') or 0)
        other = int(s.get('other_deductions') or 0)
        ch = s.get('channel', '기타')
        online_total += gross
        online_commission += comm
        online_by_channel[ch] += gross
        online_deductions += coupon + point + other
        online_by_channel_net[ch] += net

    # 플랫폼 매출 세금계산서 제외 목록 (api_settlements에 이미 집계된 매출 이중집계 방지)
    # 네이버/쿠팡 정산 관련 세금계산서 → 수수료(purchase)로 입력돼야 하지만, 혼입 방지
    _PLATFORM_BUYERS = {
        '쿠팡(주)', '쿠팡주식회사', '쿠팡 주식회사',
    ...
    for inv in tax_sales:
        ...
        if vendor in _PLATFORM_BUYERS:
            logger.debug(f'[P&L] B2B 세금계산서 제외 (플랫폼): vendor={vendor}, amt={amt:,}')
            continue
        b2b_total += amt
        b2b_by_vendor[vendor] += amt
    if b2b_total:
        logger.info(f'[P&L] B2B 세금계산서 매출: {b2b_total:,}원, 거래처: {list(b2b_by_vendor.keys())}')
    ...
    return {
        'total': online_total + b2b_total,
        'online_total': online_total,           # gross_sales 기준 총매출
        'b2b_total': b2b_total,
        'by_channel': dict(online_by_channel),  # gross_sales 채널별
        'by_channel_net': dict(online_by_channel_net),  # net_settlement 채널별 (참고)
        'b2b_by_vendor': dict(b2b_by_vendor),
        'total_commission': online_commission,
        'total_deductions': online_deductions,  # 쿠폰+포인트+기타 (계산서 없는 차감)
        'total_qty': 0,
        'data_source': 'settlement',
    }


# 세금계산서 매입 중 판관비 성격 업체 — COGS에서 제외
# (매출을 net_settlement 기준으로 잡으므로 수수료는 이미 차감됨)
# 광고비·택배비는 판관비로 별도 표시 (COGS 아님)
_COMMISSION_VENDORS = frozenset({
    # 플랫폼 수수료
    '쿠팡 주식회사', '쿠팡주식회사', '쿠팡(주)',
    '네이버파이낸셜 주식회사', '네이버파이낸셜주식회사',
    '주식회사 카카오', '카카오페이', '(주)카카오',
    '주식회사 토스페이먼츠', '토스페이먼츠',
    # 광고비
    '네이버 주식회사', '네이버주식회사',
    '주식회사 카카오모먼트',
    # 택배·물류 (아래 업체는 판관비로 별도 집계)
    '씨제이대한통운㈜', 'CJ대한통운', '씨제이대한통운 주식회사',
    '한진택배', '주식회사 한진', '롯데글로벌로지스',
    '우체국', '로젠택배', '㈜로젠',
})
...
def _calc_cogs_v2(db, date_from, date_to, data, revenue_qty=0):
    ...
    매출을 net_settlement 기준으로 사용하므로:
    - 수수료·광고비·택배비 세금계산서(_COMMISSION_VENDORS)는 COGS에서 제외
    - 원재료·포장재·제조 관련 세금계산서만 COGS로 집계
    - 제외된 항목은 'non_cogs_by_vendor'로 참고 반환 (판관비 UI 표시용)
    ...
    if agg:
        ...
        purchase_total_raw = int(pur.get('purchase_total', 0) or 0)
        by_vendor_raw = {k: int(v or 0) for k, v in (pur.get('by_vendor') or {}).items()}

        # RPC 결과에서도 판관비 업체 분리
        non_cogs = {}
        cogs_by_vendor = {}
        for v, amt in by_vendor_raw.items():
            if v in _COMMISSION_VENDORS:
                non_cogs[v] = amt
            else:
                cogs_by_vendor[v] = amt
        purchase_total = sum(cogs_by_vendor.values())
        non_cogs_total = sum(non_cogs.values())

        return {
            'total': purchase_total,
            'purchase_invoice_total': purchase_total_raw,
            'by_vendor': cogs_by_vendor,
            'non_cogs_by_vendor': non_cogs,
            'non_cogs_total': non_cogs_total,
            'actual_cost': 0,
            'estimated_cost': 0,
    ...
    non_cogs_by_vendor = defaultdict(int)

    for inv in tax_purchases:
        if inv.get('status') == 'cancelled':
            continue
        amt = int(inv.get('supply_cost_total') or 0)
        vendor = (inv.get('supplier_corp_name') or '기타')
        if vendor in _COMMISSION_VENDORS:
            non_cogs_by_vendor[vendor] += amt
            non_cogs_total += amt
        else:
            by_vendor[vendor] += amt
            purchase_total += amt

    if purchase_total == 0 and non_cogs_total == 0:
        legacy = _calc_cogs(db, date_from, date_to, revenue_qty)
        legacy['data_source'] = 'legacy'
        legacy['purchase_invoice_total'] = 0
        legacy['by_vendor'] = {}
        legacy['non_cogs_by_vendor'] = {}
        legacy['non_cogs_total'] = 0
        return legacy

    return {
        'total': purchase_total,
        'purchase_invoice_total': purchase_total + non_cogs_total,
        'by_vendor': dict(by_vendor),
        'non_cogs_by_vendor': dict(non_cogs_by_vendor),
        'non_cogs_total': non_cogs_total,
        'actual_cost': 0,
        'estimated_cost': 0,
...
def _calc_sga_v2(data, commission_from_revenue):
    ...
    # ── 차감항목(쿠폰/포인트/기타) — gross_sales 기준 매출에서 차감해야 순매출 ──
    # 계산서 없는 정산 차감 (쿠폰할인·포인트사용·기타조정)
    # 폴백 경로: _calc_revenue_v2에서 이미 total_deductions로 집계됨 → 여기서 재집계
    # RPC 경로: settlements에서 직접 집계
    promo_deduction = 0
    promo_by_channel = defaultdict(int)
    settlements_for_promo = data.get('settlements', [])
    for s in settlements_for_promo:
        sid = s.get('settlement_id', '') or ''
        if not any(sid.startswith(p) for p in _SETTLE_PREFIXES):
            continue
        ch = s.get('channel', '기타') or '기타'
        coupon = int(s.get('coupon_discount') or 0)
        point = int(s.get('point_discount') or 0)
        # 쿠팡 Wing other_deductions = 선입금/예치금 → 실제 차감 아님, 제외
        # (maesil-total 자체 wsettle_ 파서도 other_deductions=0으로 처리)
        if '쿠팡' in ch and '로켓' not in ch:
            other = 0
        else:
            other = int(s.get('other_deductions') or 0)
        total_ded = coupon + point + other
        if total_ded > 0:
            promo_deduction += total_ded
            promo_by_channel[ch] += total_ded
    if promo_deduction:
        logger.info(f'[P&L] 차감항목(쿠폰+포인트+기타): {promo_deduction:,}원')

    sga = {
        'total': total_sga + total_mfg + promo_deduction,  # 차감항목 포함 (영업이익 계산용)
        ...
        # 프로모션비용 (쿠폰/포인트 — 계산서 없는 정산 차감)
        'promo_deduction': promo_deduction,
        'promo_by_channel': dict(promo_by_channel),
...
def calculate_monthly_pnl(db, year_month, maesil_sb=None, maesil_op_id=None):
    ...
    # 5. 영업이익
    # non_cogs: 수수료·광고비·택배비 세금계산서 (원재료 COGS에서 분리, 별도 차감)
    non_cogs_total = cogs.get('non_cogs_total', 0)
    # 차감항목(쿠폰+포인트+기타)은 sga['total']에 promo_deduction으로 이미 포함됨
    operating_profit = gross_profit - sga['total'] - non_cogs_total
    ...
    # P&L 구조 명세 (UI 표시용)
    deductions_total = sga.get('promo_deduction', 0)  # 쿠폰+포인트+기타 (계산서 없는 차감)
    ...
    return {
        ...
        'deductions_total': deductions_total,          # 차감항목 (쿠폰+포인트+기타)
        'cogs': cogs,
        'non_cogs_total': non_cogs_total,              # 수수료·광고비·택배비 세금계산서
        'non_cogs_by_vendor': cogs.get('non_cogs_by_vendor', {}),
