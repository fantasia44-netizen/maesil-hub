...
def add_trade():
    ...
    mfg_date = request.form.get('manufacture_date', '').strip()
...
@trade_bp.route('/api/batches')
@role_required('admin', 'ceo', 'manager', 'sales', 'general')
def api_batches():
    """특정 창고의 특정 품목에 대한 배치(제조일자별) 목록 반환.

    Query params: location, product_name
    Returns: [{manufacture_date, qty, expiry_date}, ...]
    """
    location = request.args.get('location', '').strip()
    product_name = request.args.get('product_name', '').strip()
    if not location or not product_name:
        return jsonify([])
    try:
        ...
        info = snapshot_lookup(snapshot, product_name)
        batches = []
        for g in info.get('groups', []):
            if g.get('qty', 0) > 0:
                batches.append({
                    ...
                    'unit': g.get('unit', info.get('unit', '개')),
                })
