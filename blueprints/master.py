...
def api_products():
    ...
    # material_type 필터 (기본: 완제품만 — 상품마스터는 영업부 전용)
    # 'all' = 전체, 'finished' = 완제품만 (기본), 'none' = 분류 미지정
    mtype_filter = request.args.get('material_type', 'finished')
    page = max(int(request.args.get('page', 1)), 1)
    per_page = min(int(request.args.get('per_page', 50)), 200)
    ...
    # material_type 필터 (상품마스터는 완제품만 표시가 기본)
    if mtype_filter == 'finished':
        all_data = [r for r in all_data
                    if (r.get('material_type', '') or '').strip() == '완제품']
    elif mtype_filter == 'none':
        all_data = [r for r in all_data
                    if not (r.get('material_type', '') or '').strip()]
    # 'all'은 필터 없음
...
@master_bp.route('/api/product/rename/preview', methods=['POST'])
@role_required('admin', 'manager')
def api_product_rename_preview():
    """rename 실행 전 영향도 미리보기 (read-only).

    Body: {"old_name", "new_name"}
    Returns: {before: {old_name counts}, conflict: {new_name counts}, will_change: {table: count}}
    """
    data = request.get_json(silent=True) or {}
    old_name = str(data.get('old_name') or '').strip()
    new_name = str(data.get('new_name') or '').strip()
    if not old_name:
        return jsonify({'error': 'old_name 필요'}), 400

    db = get_db()
    tables = ['product_costs', 'stock_ledger', 'daily_revenue',
              'order_transactions', 'option_master', 'products',
              'price_master']

    def count_for(tbl, name):
        try:
            r = db.client.table(tbl).select('id', count='exact') \
                .eq('product_name', name).limit(1).execute()
            return int(r.count) if getattr(r, 'count', None) is not None else len(r.data or [])
        except Exception as e:
            msg = str(e)
    ...
    before = {t: count_for(t, old_name) for t in tables}
    conflict = {t: count_for(t, new_name) for t in tables} if new_name else {}

    return jsonify({
        'old_name': old_name,
        'new_name': new_name,
        'before': before,
        'conflict_with_new': conflict,
        'will_change_count': {k: v for k, v in before.items() if v},
    })


@master_bp.route('/api/product/rename', methods=['POST'])
@role_required('admin', 'manager')
def api_product_rename():
    """상품명 변경 + 관련 이력 일괄 전파.

    db.rename_product() 호출:
      - product_costs: 신규 INSERT + 기존 DELETE (UNIQUE 충돌 회피)
      - stock_ledger / daily_revenue / order_transactions / option_master / price_master / bom
      - api_orders.raw_data (JSONB) 는 보존 (수집 시점 원본 유지)

    audit log에 역방향 rename 컨텍스트 저장 → /admin/logs/<id>/revert 로 롤백.
    ...
    if not old_name or not new_name:
        return jsonify({'error': '원 품목명과 새 품목명을 모두 입력하세요.'}), 400
    ...
    try:
        ...
        if old_name not in cost_map and old_name.replace(' ', '') not in cost_map:
            return jsonify({'error': f'품목 "{old_name}"을 찾을 수 없습니다.'}), 404
        if new_name in cost_map or new_name.replace(' ', '') in cost_map:
            return jsonify({'error': f'"{new_name}"은(는) 이미 등록된 품목입니다.'}), 400

        # rename 실제 실행 (이력 전파)
        rename_result = db.rename_product(old_name, new_name)
        if not rename_result.get('success'):
            return jsonify({
                'error': rename_result.get('error') or '이름 변경 전파 실패',
                'changed': rename_result.get('changed', {}),
                'errors': rename_result.get('errors', []),
            }), 500

        # 롤백용 상세 컨텍스트를 audit log에 저장
        _log_action(
            'rename_product',
            target=f'{old_name} → {new_name}',
            detail=f"반영: {rename_result.get('changed')}",
