...
class InventoryRepo(BaseRepo):
    ...
    def insert_stock_ledger(self, payload_list):
        ...
        # category/unit/storage_method 자동 보완 — product_costs에서 가져와 누락 필드 채움
        # (같은 품목이 여러 category로 섞이는 데이터 오염 방지)
        payload_list = self._enrich_from_product_costs(payload_list)
    ...
    def _enrich_from_product_costs(self, payload_list):
        """stock_ledger INSERT 전, 품목마스터(product_costs)에서
        category/unit/storage_method 등 누락 필드를 자동 보완.
        같은 품목이 여러 category로 섞이는 데이터 오염 방지.
        """
        if not payload_list:
            return payload_list
        ...
        # product_costs에서 한번에 조회
        cost_map = {}
        try:
            res = self.client.table('product_costs') \
                .select('product_name,category,unit,storage_method,food_type,material_type') \
                .in_('product_name', list(names)).execute()
            for r in (res.data or []):
                cost_map[r['product_name']] = r
        except Exception as e:
            print(f'[stock_ledger enrich] product_costs 조회 실패: {e}')
            return payload_list

        # 보완: payload의 빈 필드만 덮어씀 (명시된 값은 건드리지 않음)
        for row in payload_list:
            pn = row.get('product_name', '')
            ...
            if not (row.get('category', '') or '').strip():
                mcat = (info.get('category') or '').strip()
                if mcat:
                    row['category'] = mcat
            if not (row.get('unit', '') or '').strip():
                munit = (info.get('unit') or '').strip()
                if munit:
                    row['unit'] = munit
            if not (row.get('storage_method', '') or '').strip():
                msm = (info.get('storage_method') or '').strip()
                if msm:
                    row['storage_method'] = msm
            if not (row.get('food_type', '') or '').strip():
                mft = (info.get('food_type') or '').strip()
                if mft:
                    row['food_type'] = mft
        return payload_list
    ...
    def upsert_stock_ledger_idempotent(self, payload_list):
        ...
        # ★ RPC 사용: 1000행 limit 회피 (대량 INSERT 시 중복 누락 차단)
        if with_uid:
            existing_uids = set()
            try:
                ...
                try:
                    ...
                    CHUNK = 500
                    for ci in range(0, len(uid_list), CHUNK):
                        chunk_uids = uid_list[ci:ci + CHUNK]
                        res = self.client.table("stock_ledger").select("event_uid") \
                            .in_("event_uid", chunk_uids) \
                            .range(0, 9999).execute()
                        existing_uids.update(r['event_uid'] for r in (res.data or []))
            except Exception:
                pass
