...
def process_etc_outbound(db, date_str, location, items):
    ...
    shortage = []
    for item in items:
        name = canonical(item.get('product_name', ''))  # 공백제거 강제
        qty = item.get('qty', 0)
        if not name or qty <= 0:
            continue
        mfg = (item.get('manufacture_date') or '').strip()
        ...
        if mfg:
            ...
            matching = [g for g in groups
                        if str(g.get('manufacture_date') or '') == mfg]
            if not matching:
                shortage.append(
                    f"{name}: 제조일자 {mfg} 배치 없음 (재고에 해당 제조일 없음)"
                )
                continue
            available = sum(g.get('qty', 0) for g in matching)
            if available < qty:
                shortage.append(
                    f"{name} (제조일 {mfg}): 필요 {qty}, 현재고 {available} "
                    f"(부족 {qty - available})"
                )
        else:
    ...
    for item in items:
        ...
        if qty < 0:
            ...
            payload.append({
                ...
                "manufacture_date": mfg or base.get('manufacture_date', ''),
                "memo": memo_str,
                "status": "active",
            })
            in_count += 1
        ...
        if not target_groups:
            # 제조일자 지정됐는데 배치 없으면 위에서 shortage 반환됨.
            # 여기 도달하는 건 groups 자체가 없을 때만.
            payload.append({
                "transaction_date": date_str,
                "type": "ETC_OUT",
                "product_name": name,
