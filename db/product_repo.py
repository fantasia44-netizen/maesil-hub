...
class ProductRepo(BaseRepo):
    ...
    def upsert_product_cost(self, product_name, cost_price, unit='', memo='',
        ...
        material_type: '원료', '부재료', '반제품', '완제품' (미지정 시 category/cost_type으로 자동 추론)
    ...
    def upsert_product_costs_batch(self, items):
        ...
        def _infer_material_type(item):
            """material_type 자동 추론 (명시 안 된 경우).
            기본: 완제품. 예외는 category가 반제품/부재료/원료로 명시된 경우만.
            """
            mt = item.get('material_type', '')
            if mt:
                return mt
            cat = item.get('category', '')
            if cat == '반제품': return '반제품'
            if cat == '부재료': return '부재료'
            if cat == '원료': return '원료'
            return '완제품'
        ...
        for item in items:
            payload.append({
                ...
                'material_type': _infer_material_type(item),
