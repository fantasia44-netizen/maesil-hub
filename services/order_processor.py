...
from services.option_matcher import (
    build_match_key, match_option, match_option_detailed, prepare_opt_list,
)
        ...
        try:
            ...
            for i, r in target.iterrows():
                try:
                    ...
                    match, confidence, candidates = match_option_detailed(k, opt_list)

                    if match and confidence >= 100:
                        ...
                        if candidates:
                            ...
                            # 동일 key 중복 방지
                            existing = next((r_ for r_ in needs_review if r_['key'] == k), None)
                            if not existing:
                                needs_review.append({
                                    'key': k,
                                    ...
                                    'candidates': [
                                        {
                                            'product_name': c.get('품목명', ''),
                                            'original_name': c.get('원문명', ''),
                                            'match_key': c.get('Key', ''),
                                            'barcode': c.get('바코드', ''),
                                ...
                                self.log(
                                    f"[REVIEW] key='{k[:80]}' conf={confidence} "
                                    f"cand={len(candidates)}개: "
                                    f"{[c.get('품목명','') for c in candidates[:3]]}"
                                )
                        else:
                            # 후보 자체 없음 → 완전 미매칭
                            if k not in unmatched:
                                unmatched.append(k)
                                self.log(f"[UNMATCH] key='{k[:80]}' | opt_list: {len(opt_list)}건")
                except Exception:
                    continue
            ...
            if needs_review:
                if target_type in ("리얼패킹", "외부일괄"):
                    ...
                    msg = (
                        f"비슷한 옵션이 {len(needs_review)}건 발견됐지만 정확 일치가 아닙니다.\n"
                        f"오매칭 방지를 위해 각 건을 확인 후 매칭해주세요.\n"
                        f"(옵션관리 > 검토 대기 에서 처리)"
                    )
                    result['error'] = msg
    ...
    def _save_orders_to_db(self, db, channel, matched_rows, order_file, uploaded_by, total_rows,
        ...
        for row in matched_rows:
            ...
            transaction = {
                ...
                "parser_version": "1.0",
                "product_name": _canonical_pn(row.get('display_nm', '')),  # 공백제거 강제
