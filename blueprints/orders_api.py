...
@orders_api_bp.route('/api/shipping-file-history', methods=['GET'])
@role_required('admin', 'manager', 'general')
def shipping_file_history():
    """Supabase Storage order-outputs 버킷에서 날짜별 파일 목록 조회."""
    db = get_db()
    # 최근 7일치 날짜 프리픽스 순회
    from datetime import date, timedelta
    days = int(request.args.get('days', 7))
    prefix = request.args.get('date', '')  # 특정일 지정 시
    ...
    for d in dates:
        try:
            items = db.list_output_files(prefix=d, limit=200)
            for item in (items or []):
                name = item.get('name', '')
                if not name:
                    continue
                # 집계표/송장/일괄배송 파일만
                if not any(k in name for k in ['집계표', '송장', '일괄배송', '리얼패킹', 'shipping']):
                    continue
                all_files.append({
                    'date': d,
                    'path': f"{d}/{name}",
            ...
            logger.warning(f'[ShippingHistory] {d} 조회 실패: {e}')

    return jsonify({'files': all_files, 'total': len(all_files)})


@orders_api_bp.route('/api/shipping-file-download', methods=['GET'])
@role_required('admin', 'manager', 'general')
def shipping_file_download():
    """Storage 파일 서명 URL 발급 → 리다이렉트."""
    ...
    try:
        ...
        logger.error(f'[ShippingDownload] 오류: {e}', exc_info=True)
