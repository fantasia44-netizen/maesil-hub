    ...
    for item in items:
        ...
        for mat in item.get('materials', []):
            ...
            if not groups:
                ...
                # 매칭 배치 없으면 경고 (조용한 FIFO 방지)
                if not matched:
                    warnings.append(
                        f"[{loc}] {mat_name}: 제조일 {mat_mfg_date} 재고 없음 — FIFO로 대체 차감"
                    )
                ...
                for g in ordered:
                    ...
                    if deduct <= 0:
                        continue
                    # 매칭 배치에서 차감 시 g의 날짜, FIFO 대체 시 사용자 지정 날짜 기록
                    recorded_mfg = g.get('manufacture_date', '') or mat_mfg_date
                    payload.append({
                        "transaction_date": date_str,
                        "type": "PROD_OUT",
                        "product_name": mat_name,
