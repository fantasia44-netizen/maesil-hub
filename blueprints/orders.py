...
def process():
    ...
    try:
        ...
        # 미매칭 또는 부분매칭 발견 → 모달 팝업으로 등록/확정 유도
        if result.get('unmatched') or result.get('needs_review'):
            # 재처리를 위해 세션에 컨텍스트 보관
            ...
            return render_template('orders/index.html',
                                   unmatched_items=result.get('unmatched') or [],
...
def api_product_list():
    ...
    try:
        ...
        names = []
        rpc_ok = False
        # RPC 우선 — DB-side DISTINCT, 1000행 limit 회피
        try:
            rpc_res = db.client.rpc('rpc_get_stock_distinct_products', {}).execute()
            names = sorted(set(
                r['product_name'] for r in (rpc_res.data or [])
        ...
        if not rpc_ok:
            # fallback: 전체 active stock_ledger의 product_name distinct 수집 (페이지네이션)
            all_rows = []
            offset = 0
            PAGE = 1000
            while True:
                ...
                offset += PAGE
            names = sorted(set(
                r['product_name'] for r in all_rows
                if r.get('product_name')
            ))
...
def api_rocket_manual():
    ...
    db = get_db()
    username = current_user.username if current_user.is_authenticated else ''
    # ★ 가격등록 직후 캐시 stale 방지: 입력 시점마다 fresh 가격 사용
    db._invalidate_price_cache()
    price_map = db.query_price_table()

    revenue_payload = []
    stock_items = []
    missing_prices = []  # 가격 0원인 품목 추적
    ...
    for item in items:
        ...
        # ★ 0원 저장 차단 — silent fail 방지 (단호박2p 등 사고 재발 방지)
    ...
    # ★ 가격 누락 품목이 있으면 어떤 것도 저장하지 않고 에러 반환
    #   (혼합 처리 시 일부만 저장되어 나중에 추적 불가능 → 전부 거절이 안전)
    if missing_prices:
        return jsonify({
            'error': f'로켓판매가가 0원이거나 미등록인 품목이 있습니다: {", ".join(missing_prices)}. '
                     f'상품관리 → 로켓 가격 등록 후 재시도하세요.',
    ...
    try:
        # ★ 정책 변경 (2026-04-27): 이중 stock_ledger 경로 제거
        #   - 이전: upsert_revenue → _auto_stock_from_revenue (재고무시 SALES_OUT) → process_single_outbound (재고검증)
        #     → 같은 입력에 SALES_OUT 2번 시도 + 정합성 모순
        #   - 변경: 1) 재고검증 먼저, 2) 통과 시에만 daily_revenue + SALES_OUT 일괄 저장
        #   - skip_auto_stock_categories=['로켓'] 로 _auto_stock_from_revenue 우회

        # 1. 재고 사전 검증 (저장 전)
        from services.outbound_service import _load_stock_snapshot
        from services.excel_io import snapshot_lookup
        stock_snapshot = _load_stock_snapshot(db, warehouse)
        shortage = []
        for s in stock_items:
            snap = snapshot_lookup(stock_snapshot, s['product_name'])
            avail = snap.get('total', 0)
            if avail < s['qty']:
                shortage.append(f"{s['product_name']}: 요청 {s['qty']}, 재고 {avail} (부족 {s['qty']-avail})")

        if shortage:
            return jsonify({
                'error': f'재고 부족으로 저장이 거부되었습니다 ({warehouse}): {", ".join(shortage[:5])}',
                'shortage': shortage,
            }), 400

        # 2. 매출 + 재고차감 (둘 다 통과 시에만)
        db._retry_on_disconnect(
            db.upsert_revenue, revenue_payload,
            skip_auto_stock_categories=['로켓']  # 아래 process_single_outbound가 처리하므로 자동연동 스킵
        )
        ...
        try:
            ...
            if result.get('success'):
                ...
                # 사전 검증 통과 후 실패 — 동시성 문제 등 (드문 케이스)
                shortage = result.get('shortage', [])
                stock_msg = f' ⚠️ 재고차감 실패(매출은 기록됨): {", ".join(shortage[:3])}'
                stock_critical = True
                current_app.logger.error(
                    f'[로켓수동입력 재고차감 실패-사전검증후] {revenue_date} | {warehouse} | shortage={shortage}'
                )
        except Exception as stk_err:
            stock_msg = f' ⚠️ 재고차감 실패(매출은 기록됨): {stk_err}'
            stock_critical = True
...
def api_rocket_add_product():
    ...
    try:
        ...
        if not existing_p.data:
            # 정규화 불일치지만 product_name 정확일치 체크 (23505 방어)
            existing_p = db.client.table('products').select('product_name') \
        ...
        if '23505' in err_str or 'duplicate key' in err_str.lower():
            return jsonify({'success': False, 'error': f'이미 등록된 상품명입니다: "{name}"'})
...
def api_rocket_manual_update(rev_id):
    ...
    try:
        ...
        if action == 'delete' or (new_qty is not None and int(new_qty) == 0):
            # ★ 멱등성: 이미 삭제 처리된 daily_revenue 면 중복실행 차단
            #   (사용자가 버튼 여러번 누르는 사고 방지 — 재고 다중복원 차단)
            stock_msg = ''
            try:
                from services.product_name import canonical
                ...
                # ★ 핵심 수정: SALES_RETURN INSERT 대신 원본 SALES_OUT을 cancel
                #   (DR_AUTO event_uid 패턴으로 매칭)
                if old_product and old_qty > 0 and old_date:
                    cancelled = 0
                    # daily_revenue 자동연동으로 생성된 SALES_OUT 매칭
                    matches = db.client.table('stock_ledger').select('id,status') \
                        .eq('transaction_date', old_date) \
                        .eq('type', 'SALES_OUT') \
                        .eq('product_name', old_product) \
                        .eq('location', old_warehouse) \
                        .eq('status', 'active') \
                        .like('event_uid', f'DR_AUTO:{old_date}:%:로켓') \
                        .range(0, 9999) \
                        .execute().data or []
                    for m in matches:
                        ...
                        cancelled += 1
                    if cancelled:
                        stock_msg = f', 출고 {cancelled}건 cancelled({old_warehouse})'
                    else:
                        stock_msg = f' (대응 SALES_OUT 없음 — 이미 정리됨)'
            except Exception as stk_err:
                stock_msg = f' (재고원복 실패: {stk_err})'
                current_app.logger.warning(f'로켓 삭제 재고원복 오류: {stk_err}')
        ...
        # 매출일자 (ex 26→27 수정)
        new_date = data.get('revenue_date')
        if new_date and new_date != old_row.get('revenue_date'):
            update_data['revenue_date'] = new_date
        ...
        # 재고 조정 — 상품/수량/창고/일자 변경 시 stock_ledger 반영
        # ★ 정책: 원본 DR_AUTO SALES_OUT을 cancel + 새 SALES_OUT INSERT (SALES_RETURN 사용 X)
        stock_msg = ''
        try:
            old_product = canonical(old_row.get('product_name', ''))
            old_qty = int(old_row.get('qty', 0))
            ...
            final_date = update_data.get('revenue_date', old_date)
            ...
            if product_changed or warehouse_changed or qty_changed or date_changed:
                # 1) 원본 DR_AUTO SALES_OUT cancel (있으면)
                cancelled = 0
                try:
                    matches = db.client.table('stock_ledger').select('id') \
                        .eq('transaction_date', old_date) \
                        .eq('type', 'SALES_OUT') \
                        .eq('product_name', old_product) \
                        .eq('location', old_warehouse) \
                ...
                # 2) 새 SALES_OUT INSERT (final 값 기준)
                if final_qty > 0 and final_product:
                    db.client.table('stock_ledger').insert({
                        ...
                        'memo': f'로켓매출 수정 (rev#{rev_id}) — orig cancelled={cancelled}',
                        'created_by': username,
                        'event_uid': f'DR_AUTO:{final_date}:{final_warehouse}:{final_product}:로켓:rev{rev_id}',
                    }).execute()
                stock_msg = (f', 출고 cancelled={cancelled} → 재차감 {final_product}'
                             f' qty={final_qty} {final_warehouse} {final_date}')
        except Exception as stk_err:
...
def api_products_add():
    ...
    try:
        # 1) 정확한 이름 중복 체크
        dup_exact = db.client.table('products').select('product_name') \
            .eq('product_name', name).limit(1).execute()
        if dup_exact.data:
            return jsonify({'ok': False, 'error': f'이미 등록된 상품입니다: "{name}"'})

        # 2) 정규화된 이름 중복 체크 (띄어쓰기 다른 동일 상품)
        ...
        err_str = str(e)
        if '23505' in err_str or 'duplicate key' in err_str.lower():
            return jsonify({'ok': False, 'error': f'이미 등록된 상품명입니다: "{name}"'})
    ...
    try:
        ...
        if product_name:
            ...
            if p.data:
                ...
                # products에 없으면 신규 추가 (upsert로 중복 방어)
                db.client.table('products').upsert({
                    'product_name': product_name,
                    'name_normalized': name_norm,
