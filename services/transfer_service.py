...
def process_manual_transfer(db, product_name, qty, from_location, to_location,
                            date_str, lot_number=None, grade=None,
                            manufacture_date=None, created_by=None,
                            shared_stock=None):
    """수동 창고 이동 (FIFO 자동 상속).
    ...
    remain = int(qty)
    name = canonical(product_name)  # 공백제거 강제
    src = normalize_location(from_location)
    dst = normalize_location(to_location)

    warnings = []

    # 재고 스냅샷 — 호출자가 shared_stock 전달 시 재사용 (batch 성능+메모리 절감)
    # 전달 dict 구조: {normalized_location: snapshot_dict}
    if shared_stock is not None and src in shared_stock:
        stock = shared_stock[src]
    else:
        stock = _load_stock_snapshot(db, src)
        if shared_stock is not None:
            shared_stock[src] = stock
    ...
    if not groups:
        ...
        # 제조일 지정 시 해당 배치 우선, 나머지 FIFO
        if req_mfg:
            matched = [g for g in groups if str(g.get('manufacture_date', '')).strip() == req_mfg]
            others  = [g for g in groups if str(g.get('manufacture_date', '')).strip() != req_mfg]
            if not matched:
                warnings.append(f"[{src}] {name}: 제조일 {req_mfg} 재고 없음 — FIFO로 대체 이동")
            groups = matched + others
        ...
        for g in groups:
            ...
            recorded_mfg = g.get('manufacture_date', '') or req_mfg
            payload.append({
                "transaction_date": date_str, "type": "MOVE_OUT",
                "product_name": name,
            ...
            g['qty'] -= deduct  # shared_stock 재사용 시 다음 iteration에 반영
            remain -= deduct
