...
def revert_audit_log(log_id):
    ...
    try:
        ...
        if action == 'update_product_cost':
            ...
            # 상품명 변경 롤백 — new_name 상태로 돌아간 것을 old_name으로 역방향 rename
            # old_value: {product_name: old, reverse_to: old, changed: {...}}
            # new_value: {product_name: new, changed: {...}}
            old_pn = old_value.get('reverse_to') or old_value.get('product_name')
            new_pn = (new_value or {}).get('product_name')
            # target 포맷이 "old → new" 형식이면 파싱해서 보정
            if not new_pn and target and '→' in target:
                try:
                    parts = [p.strip() for p in target.split('→')]
                    if len(parts) == 2:
                        old_pn = old_pn or parts[0]
                        new_pn = parts[1]
                except Exception:
                    pass
            if not old_pn or not new_pn:
                return jsonify({
                    'error': 'rename 롤백: old/new 이름을 찾을 수 없습니다.'
                }), 400
            # 역방향 실행: new → old
            rev = db.rename_product(new_pn, old_pn)
            if not rev.get('success'):
                return jsonify({
                    'error': f'롤백 실패: {rev.get("error")}',
