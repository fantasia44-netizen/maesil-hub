...
def cj_generate_invoices():
    ...
    try:
        ...
        auto_push = request.form.get('auto_push', 'true').lower() != 'false'
        ...
        # 성공 건 채널별로 그룹화 → 마켓 자동 push
        push_summary = {}
        if auto_push and result.get('success', 0) > 0:
            # order_no → channel 매핑
            ...
            for r in result.get('results', []):
                if r.get('ok'):
                    ono = r['order_no']
                    ch = order_channel_map.get(ono) or channel or ''
                    if ch:
            ...
            for ch, order_nos in by_channel.items():
                try:
                    push_res = push_invoices(
                        db, mgr, ch,
                        triggered_by=current_user.username,
                        order_nos=order_nos,
                    )
                    push_summary[ch] = {
                        'total': push_res.get('total', 0),
                        'success': push_res.get('success', 0),
                        'failed': push_res.get('failed', 0),
                    }
                    _log_action(
                        f'마켓 송장 자동전송: {ch} — '
                        f'{push_res.get("success", 0)}/{push_res.get("total", 0)}건 성공'
                    )
                except Exception as push_err:
                    logger.error(f'[CJ Generate] {ch} push 오류: {push_err}', exc_info=True)
                    push_summary[ch] = {'error': str(push_err)}

        result['push_summary'] = push_summary
...
def api_label_print():
    ...
    # 발송인 정보 — my_business DB 조회
    sender = db.get_cj_sender() or {}
...
def _query_shipments_by_invoice(db, invoice_nos: list) -> list:
    ...
    try:
        ...
        for i in range(0, len(invoice_nos), 50):
            ...
            try:
                rpc_res = db.client.rpc('rpc_search_order_shipping_by_invoice', {
                    'p_invoices': list(chunk),
                }).execute()
                if rpc_res.data:
                    results.extend(rpc_res.data)
            except Exception:
