...
def save_config(channel):
    ...
    # ── 저장 전 연결 검증 ──
    has_store_keys = bool(payload.get('client_id') and payload.get('client_secret'))
    has_ad_keys = bool(extra.get('ad_customer_id') and extra.get('ad_api_key')
                       and extra.get('ad_secret_key'))

    # 네이버 광고 API 키만 입력한 경우 — 광고 연결 테스트
    if is_naver(channel) and has_ad_keys and not has_store_keys:
        try:
            from services.marketplace.naver_ad_client import NaverAdClient
            ad_client = NaverAdClient(extra)
            campaigns = ad_client.get_campaigns()
            if campaigns is None:
                flash('네이버 검색광고 API 연결 실패 — 고객번호/키를 확인하세요. 설정이 저장되지 않았습니다.', 'danger')
                return redirect(url_for('marketplace.index'))
        except Exception as e:
            flash(f'네이버 검색광고 API 연결 실패: {e}. 설정이 저장되지 않았습니다.', 'danger')
            return redirect(url_for('marketplace.index'))

    # 스마트스토어 키가 있으면 토큰 발급 테스트
    if is_naver(channel) and has_store_keys:
        ...
        if client:
            ...
            test_client = NaverCommerceClient(test_cfg)
            token_ok = test_client.refresh_token(db)
            if not token_ok:
                flash(f'{channel} 스마트스토어 API 연결 실패 — client_id/secret을 확인하세요. 설정이 저장되지 않았습니다.', 'danger')
                return redirect(url_for('marketplace.index'))
    ...
    if is_naver(channel):
        ...
        if has_ad_keys:
            parts.append('검색광고')
        label = ' + '.join(parts) if parts else channel
        flash(f'{label} API 설정이 저장되었습니다.', 'success')
    else:
...
def sales_data():
    ...
    # 3) Maesil Insight 정산 병합 (브릿지 활성 시)
    maesil_sb = getattr(current_app, 'maesil_sb', None)
    if maesil_sb:
        from services.maesil_bridge import (
            get_maesil_operator_id, get_maesil_settlements,
            get_maesil_orders, merge_settlements, merge_orders,
        )
        maesil_op_id = get_maesil_operator_id()
        if maesil_op_id:
            m_settlements = get_maesil_settlements(maesil_sb, maesil_op_id, date_from, date_to)
            m_orders = get_maesil_orders(maesil_sb, maesil_op_id, date_from, date_to)
            settlements = merge_settlements(settlements, m_settlements)
            orders = merge_orders(orders, m_orders)
