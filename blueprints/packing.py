...
def api_lookup_barcode():
    ...
    try:
        res = db.client.table("order_transactions") \
            ...
            .eq("status", "정상").range(0, 9999).execute()
...
def _process_outbound(db, job):
    ...
    try:
        ...
        try:
            rpc_res = db.client.rpc('rpc_get_packing_pending_orders', {
                'p_channel': channel,
                'p_order_nos': list(order_nos),
            }).execute()
            ot_ids = [r['id'] for r in (rpc_res.data or [])]
        except Exception:
...
def api_courier_register():
    ...
    # 발송인 정보 — my_business DB 조회 (환경변수 폴백)
    sender = db.get_cj_sender() or {
        'name': os.environ.get('CJ_SENDER_NAME', ''),
        'phone': os.environ.get('CJ_SENDER_PHONE', ''),
        'zipcode': os.environ.get('CJ_SENDER_ZIPCODE', ''),
        'address': os.environ.get('CJ_SENDER_ADDRESS', ''),
        'detail_address': os.environ.get('CJ_SENDER_DETAIL', ''),
    }
