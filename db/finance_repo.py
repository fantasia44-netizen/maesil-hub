...
class FinanceRepo(BaseRepo):
    ...
    def sum_revenue_by_date(self, date_str):
        ...
        ★ RPC rpc_get_revenue_by_date 우선 사용 (1000행 limit 회피).
        Returns: dict {total_amount, settlement, commission, qty}
        """
        # RPC 우선 (DB 레벨 합산 → 정확성 보장)
        try:
            rpc_res = self.client.rpc('rpc_get_revenue_by_date',
                                      {'p_date': date_str, 'p_channel': None}).execute()
            data = rpc_res.data or {}
            if data:
                ot_total = data.get('ot_total_amount', 0)
                ot_list = data.get('order_transactions') or []
                # RPC 결과에서 직접 합산
                return {
                    'total_amount': sum(r.get('total_amount', 0) or 0 for r in ot_list),
                    'settlement': sum(r.get('settlement', 0) or 0 for r in ot_list),
                    'commission': sum(r.get('commission', 0) or 0 for r in ot_list),
                    'qty': sum(r.get('qty', 0) or 0 for r in ot_list),
                }
        except Exception as e:
            print(f"[DB] sum_revenue_by_date RPC fallback: {e}")

        # Fallback: 페이지네이션 직접 조회 (RPC 미배포 시)
        try:
            all_rows = []
            offset = 0
            PAGE = 1000
            while True:
                ...
                offset += PAGE
            return {
                'total_amount': sum(r.get("total_amount", 0) or 0 for r in all_rows),
                'settlement': sum(r.get("settlement", 0) or 0 for r in all_rows),
                'commission': sum(r.get("commission", 0) or 0 for r in all_rows),
                'qty': sum(r.get("qty", 0) or 0 for r in all_rows),
            }
        except Exception:
